...
def frictionless_moment(InfoMat, data_matrix, m, l, dt):
    ...
    sa_x = np.array(sa_matrix[:, :, 0])
    sv_y = np.array(sv_matrix[:, :, 1])
    ...
    row_sums_monent1 = np.sum(moment1, axis=1)
    moment_modulus = np.sqrt(row_sums_x ** 2 + row_sums_y ** 2)  # 原来的
    moment1 = row_sums_monent1 + moment_modulus
    moment1 = moment1[moment1 <= 80000]
    filter_moment = moment1 * 0.01

    # filter_moment = [num for num in moment_modulus if num <= 80000]
    # print(row_sums_x)
    # print(row_sums_y)
...
def simulation(l1, l2, l3_left, l3_right, l4, l5, e, s1, theta1_deg, radian_value):
    ...
    # 求不考虑摩擦下的力矩
    frictionless_moments = frictionless_moment(InfoMat, data_matrix, m, l, dt=1)
    ...
    va_fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(12, 8))
    ...
    ax3.set_title('Moment of Point')
    ax3.set_xlabel('Time (frames)')
    ax3.set_ylabel('Moment (units per second squared)')
    ax3.legend()
    ax3.set_xlim(0, len(frictionless_moments) - 4)  # 设置x轴范围
    # ax3.set_ylim(-0.02, 0.02)  # 设置y轴范围
